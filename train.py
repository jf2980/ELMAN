...
import def_loss.custom_loss as custom_loss
#import loss_var
#change the util file scale
...
if __name__ == '__main__':
    ...
    from datas.df2_utils import create_datasets
    ...
    try:
        model = import_module('models.{}_network'.format(args.model, args.model)).create_model(args)
    except Exception:
        raise ValueError('not supported model type! or something')
    model = nn.DataParallel(model).to(device)

    ## definition of loss and optimizer
    #loss_func = loss_var.CharbonnierLoss() +
    loss_func = custom_loss.CustomLoss()
    ...
    if args.resume is not None:
        ...
        timestamp = utils_elan.cur_timestamp_str()
        ...
        log_name = os.path.join(experiment_path, 'log.txt')
        stat_dict = utils_elan.get_stat_dict()
    ...
    sys.stdout = utils_elan.ExperimentLogger(log_name, sys.stdout)
    log_dir = os.path.join(args.log_path, 'ELARN_X21101', 'log')
    utils.mkdir(log_dir)
    writer = SummaryWriter(log_dir=log_dir, filename_suffix=f'_ {"ELARN_X21101"}')
    print(model)
    sys.stdout.flush()
        ...
        if epoch % args.test_every == 0:
            ...
            for valid_dataloader in valid_dataloaders:
                ...
                for lr, hr in tqdm(loader, ncols=80):
                    ...
                    # conver to ycbcr
                    if args.colors == 3:
                        hr_ycbcr = utils_elan.rgb_to_ycbcr(hr)
                        sr_ycbcr = utils_elan.rgb_to_ycbcr(sr)
                        hr = hr_ycbcr[:, 0:1, :, :]
                        sr = sr_ycbcr[:, 0:1, :, :]
                    ...
                    psnr = utils_elan.calc_psnr(sr, hr)
                    ssim = utils_elan.calc_ssim(sr, hr)
                    avg_psnr += psnr
                    avg_ssim += ssim
                ...
                writer.add_scalar(f'{name}_psnr', avg_psnr, epoch)
                writer.add_scalar(f'{name}_ssim', avg_ssim, epoch)
